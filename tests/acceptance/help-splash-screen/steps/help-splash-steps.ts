...
Given(
  'the NO_COLOR environment variable is set to {string}',
  function (this: ChromatoHelpWorld, value: string) {
...
Given(
  'the terminal is set to dumb mode with an ASCII-only locale',
  function (this: ChromatoHelpWorld) {
...
Then('the ASCII art logo appears at the top of the output', function (this: ChromatoHelpWorld) {
...
Then(
  'the output contains ANSI color sequences on the logo lines',
  function (this: ChromatoHelpWorld) {
...
Then(
  'the tagline {string} appears in the output',
  function (this: ChromatoHelpWorld, tagline: string) {
    const plain = stripAnsi(this.capturedOutput);
    assert.ok(
...
Then('the Commander help text appears below the banner', function (this: ChromatoHelpWorld) {
...
Then('the process exits with code 0', function (this: ChromatoHelpWorld) {
  assert.strictEqual(this.exitCode, 0, `Expected exit code 0 but got ${this.exitCode}`);
});

Then(
  'the first output byte arrives within {int} milliseconds',
  async function (this: ChromatoHelpWorld, limitMs: number) {
...
Then(
  'the tagline {string} appears exactly once in the output',
  function (this: ChromatoHelpWorld, tagline: string) {
    const plain = stripAnsi(this.capturedOutput);
...
Then(
  'the process exits within {int} milliseconds of first output',
  function (this: ChromatoHelpWorld, limitMs: number) {
...
Then('no ANSI escape sequences appear in the output', function (this: ChromatoHelpWorld) {
  const ansiCount = countAnsiSequences(this.capturedOutput);
  assert.strictEqual(
    ansiCount,
    0,
...
Then('the ASCII art logo is present as plain text', function (this: ChromatoHelpWorld) {
  const plain = stripAnsi(this.capturedOutput);
  assert.ok(
...
Then(
  'the tagline {string} is present as plain text',
  function (this: ChromatoHelpWorld, tagline: string) {
    const plain = stripAnsi(this.capturedOutput);
    assert.ok(
      plain.includes(tagline),
...
Then('the Commander help text is present as plain text', function (this: ChromatoHelpWorld) {
  const plain = stripAnsi(this.capturedOutput);
  assert.ok(
    plain.includes('Usage:') || plain.includes('chromato'),
...
Then(
  'the output does not contain the Unicode divider character {string}',
  function (this: ChromatoHelpWorld, dividerChar: string) {
    assert.ok(
...
Then('the tagline {string} is still present', function (this: ChromatoHelpWorld, tagline: string) {
  const plain = stripAnsi(this.capturedOutput);
  assert.ok(
    plain.includes(tagline),
...
Then('the process produces no output on stderr', function (this: ChromatoHelpWorld) {
  ...
  assert.strictEqual(stripped, '', `Expected empty stderr but got:\n${this.capturedStderr}`);
});
...
Then(
  'the descriptor and hint are rendered with dim ANSI styling',
  function (this: ChromatoHelpWorld) {
