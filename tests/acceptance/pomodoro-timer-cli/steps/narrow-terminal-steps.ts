...
When(
  'she runs {string} with a {int}-minute work session',
...
Then('the phase label and remaining time are both visible', function (this: ChromatoWorld) {
  ...
  assert.ok(hasTimer, `Expected a timer countdown (MM:SS) in output but got:\n${plainText}`);
});
...
Then(
  'no display element is truncated or overflows the {int}-column boundary',
