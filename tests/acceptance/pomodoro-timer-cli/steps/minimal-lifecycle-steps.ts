...
import { spawnChromato, runChromato, waitForOutput, readStateFile } from './helpers';
...
Given('Natasha has completed {int} full Pomodoro today', function (
  this: ChromatoWorld,
  count: number
) {
...
Given('Natasha has a {int}-minute session with {int} minutes remaining', async function (
  this: ChromatoWorld,
  totalMinutes: number,
...
Given('a second session is active with {int} minutes remaining', async function (
  this: ChromatoWorld,
  remainingMinutes: number
) {
...
Given('a work session is actively writing to the state file every 5 seconds', async function (
  this: ChromatoWorld
) {
...
Given(/^the terminal reports TERM=(.+)$/, function (
  this: ChromatoWorld,
  term: string
) {
...
When('the developer runs {string} in any display mode', async function (
  this: ChromatoWorld,
  command: string
) {
...
When('the output is captured for {int} seconds', function (
  this: ChromatoWorld,
  _seconds: number
) {
...
When('the developer runs {string} from the VS Code integrated terminal', async function (
  this: ChromatoWorld,
  command: string
) {
  const args = command.replace(/^chromato\s+/, '').split(/\s+/).filter(Boolean);
...
When('the state file is read {int} times over {int} minutes at random intervals', async function (
  this: ChromatoWorld,
  times: number,
  _minutes: number
) {
...
Then(/^the state file contains the fields: (.+)$/, function (
  this: ChromatoWorld,
  fieldList: string
) {
...
Then('the state file exists within {int} second of session start', async function (
  ...
  assert.fail(`Expected state file with active phase to appear within ${seconds} second(s) of session start`);
});

Then('every read returns valid parseable JSON', function (this: ChromatoWorld) {
  const reads = (this as ChromatoWorld & { stateFileReads: Array<Record<string, unknown> | null> }).stateFileReads;
...
Then('the chromato process is no longer listed in the running process list', function (
  this: ChromatoWorld
) {
  const isRunning =
    this.process !== null &&
  ...
  assert.ok(
    !isRunning,
    'Expected chromato process to have exited but it is still running'
  );
});

Then('no child processes spawned by chromato remain running', function (
  this: ChromatoWorld
) {
...
Then(/^each output line includes the phase \(WORK or BREAK\), remaining time, and session number$/, function (
  this: ChromatoWorld
) {
  // Filter out the interrupt summary line and other non-timer lines.
  const lines = this.capturedOutput.split('\n').filter((l) =>
    Boolean(l) && /WORK|BREAK|LONG_BREAK|OVERDUE/.test(l)
  );
  assert.ok(
    lines.length > 0,
...
Then('no full-screen TUI clearing sequences appear in the output', function (
  this: ChromatoWorld
) {
...
Then('the session uses ASCII characters because --minimal implies ASCII mode', function (
  this: ChromatoWorld
) {
  assert.ok(
...
Then(/^the output includes the current Pomodoro number \(e\.g\. ".*?"\)$/, function (
  this: ChromatoWorld
) {
  assert.match(
    this.capturedOutput,
...
Then(/^the progress indicator is present \(e\.g\. ".*?"\)$/, function (
  this: ChromatoWorld
) {
  assert.ok(
    this.capturedOutput.includes('[') && this.capturedOutput.includes(']'),
...
Then('the session starts successfully', function (this: ChromatoWorld) {
  assert.ok(
    ...
    'Expected session output but got empty string'
  );
});
...
Then('the first output line shows {string} as the remaining work time', function (
  this: ChromatoWorld,
  timeStr: string
) {
...
Then('the session configuration is correctly reflected in the plain text output', function (
  this: ChromatoWorld
) {
  assert.match(
    this.capturedOutput,
...
Then('the phase, remaining time, and session number are visible as plain text', function (
  this: ChromatoWorld
) {
...
Then('the functional session information remains present in plain text', function (
  this: ChromatoWorld
) {
  const output = stripAnsiHelper(this.capturedOutput);
...
Then('the session phase, remaining time, and session number are still present as plain text', function (
  this: ChromatoWorld
) {
  const output = stripAnsiHelper(this.capturedOutput);
...
Then('the TUI renders with Unicode block characters in the progress bar', function (
  this: ChromatoWorld
) {
...
Then(/^the phase color uses the 256-color work phase scheme \(cyan or green\)$/, function (
  this: ChromatoWorld
) {
...
Then(/^the progress bar and phase label use work-phase colors \(green or cyan range\)$/, function (
  this: ChromatoWorld
) {
  assert.ok(
    /\x1b\[/.test(this.capturedOutput),
...
Then(/^when the break phase begins the colors switch to break-phase colors \(blue or indigo range\)$/, function (
  this: ChromatoWorld
) {
...
Then(/^when overdue activates the colors switch to the overdue scheme \(red or amber range\)$/, function (
  this: ChromatoWorld
) {
...
Then(/^the progress bar fill consists of Unicode block characters \(full block or partial blocks\)$/, function (
  this: ChromatoWorld
) {
  const output = this.capturedOutput + (this.capturedStderr ?? '');
  assert.ok(
...
Then('no ASCII fallback characters appear in the filled portion of the bar', function (
  this: ChromatoWorld
) {
  const output = this.capturedOutput + (this.capturedStderr ?? '');
...
Then(/^the progress bar renders with ASCII characters \("=", "-", ">"\)$/, function (
  this: ChromatoWorld
) {
  const output = this.capturedOutput + (this.capturedStderr ?? '');
  assert.ok(
...
Then(/^all session information \(phase, time, count\) remains available$/, function (
  this: ChromatoWorld
) {
...
Then('no unhandled exception or error trace appears in the output', function (
  this: ChromatoWorld
) {
  const combined = this.capturedOutput + (this.capturedStderr ?? '');
  assert.ok(
...
Then('the output contains the Pomodoro phase and remaining time', function (
  this: ChromatoWorld
) {
...
Given('Yuki runs {string}', async function (
  this: ChromatoWorld,
  command: string
) {
  const args = command.replace(/^chromato\s+/, '').split(/\s+/).filter(Boolean);
