...
Given('the state data directory exists at the XDG data home path', function (this: ChromatoWorld) {
  const stateDir = path.join(this.tempDir, 'chromato');
  fs.mkdirSync(stateDir, { recursive: true });
});

Given(
  'a clean CI environment with Node.js {int} installed',
  function (this: ChromatoWorld, _version: number) {
    // Verify Node.js is available (already required for the test runner itself).
    const nodeVersion = process.version;
    assert.ok(
      nodeVersion.startsWith('v22') || parseInt(nodeVersion.slice(1), 10) >= 22,
      `Expected Node.js 22+ but got ${nodeVersion}`,
    );
  },
);

Given('a clean CI environment with no warm Node.js caches', function (this: ChromatoWorld) {
...
Given(
  'a chromato session is running with a {int}-minute work duration',
...
Given(
  '{int} seconds have elapsed for the process to reach steady state',
...
Given(
  'tmux {string} is installed in the CI environment',
...
Given(
  'no prior chromato process has run in the current shell session',
...
When(
  'the CI measurement script sends SIGTERM to the chromato process',
  async function (this: ChromatoWorld) {
    if (!this.process) {
...
Then(
  'the first output byte arrives within {int} milliseconds of process start',
...
Then(
  'the median cold start time is under {int} milliseconds',
  function (this: ChromatoWorld, maxMs: number) {
    assert.ok(
      this.elapsedMs <= maxMs,
...
Then('the RSS memory is less than {int} megabytes', function (this: ChromatoWorld, maxMb: number) {
...
Then(
  'the median latency is under {int} milliseconds',
  function (this: ChromatoWorld, maxMs: number) {
    assert.ok(
      this.elapsedMs <= maxMs,
...
Then(
  'the rendered output shows the correct phase and remaining time',
  function (this: ChromatoWorld) {
