...
const flickerContext = new WeakMap<
  ChromatoWorld,
  {
...
const scenarioContext = new WeakMap<
  ChromatoWorld,
  {
...
Given(
  'a {int}-second work session has just started',
  function (this: ChromatoWorld, durationSeconds: number) {
...
When('{int} seconds elapse', function (this: ChromatoWorld, seconds: number) {
  ...
  assert.ok(
    ctx,
    'Session context not initialized -- run "Given a N-second work session has just started" first',
  );
...
Then(
  'the TUI has rendered at least {int} distinct frames',
  function (this: ChromatoWorld, minFrames: number) {
    const ctx = scenarioContext.get(this);
...
Then(
  'each frame shows a higher fill percentage than the previous frame',
...
Then(
  'chromato outputs an informational message about ASCII fallback mode',
  function (this: ChromatoWorld) {
...
Then('the informational ASCII fallback message does not appear', function (this: ChromatoWorld) {
  const combined = this.capturedOutput + (this.capturedStderr ?? '');
  assert.ok(
...
Then(
  'the session runs normally with ASCII progress bar characters',
  function (this: ChromatoWorld) {
...
Given('the developer is not interacting with the chromato TUI', function (this: ChromatoWorld) {
  // No-op: documents that no user input occurs during measurement window.
});

When(
  'CPU usage is sampled over a {int}-second window',
  function (this: ChromatoWorld, seconds: number) {
...
Then(
  ...
  function (this: ChromatoWorld, maxCpuPercent: number) {
    ...
    const source = fs.readFileSync(path.resolve('src/application/sessionService.ts'), 'utf-8');
    assert.ok(
...
Given(
  'a work session is active on a {int}-column terminal',
  function (this: ChromatoWorld, columns: number) {
...
When(
  '{int} consecutive render frames are captured',
  function (this: ChromatoWorld, frameCount: number) {
    const ctx = scenarioContext.get(this);
...
Then(
  'each frame differs from the previous by at most one progress bar character',
  function (this: ChromatoWorld) {
    const flickerCtx = flickerContext.get(this);
    assert.ok(flickerCtx, 'Flicker context not initialised');
...
Then(
  'no frame shows a lower fill than the immediately preceding frame',
  function (this: ChromatoWorld) {
    const flickerCtx = flickerContext.get(this);
    assert.ok(flickerCtx, 'Flicker context not initialised');
...
Then(
  /^the progress bar fill uses the work phase color \(green or cyan\)$/,
  function (this: ChromatoWorld) {
...
Then(
  /^when the break phase begins the bar fill switches to the break phase color \(blue or indigo\)$/,
  function (this: ChromatoWorld) {
...
Then(
  'the session starts and counts down identically to Unicode mode',
  function (this: ChromatoWorld) {
...
Then(
  /^the process exit code is (\d+) \(not an error condition\)$/,
  function (this: ChromatoWorld, codeStr: string) {
...
Then('the progress bar shows {int} percent fill', function (this: ChromatoWorld, _percent: number) {
...
Then(
  /^the fill alternates between solid red and dim red on a \d+-second interval$/,
  function (this: ChromatoWorld) {
...
Then(
  'the timer shows the overdue elapsed time formatted as {string}',
  function (this: ChromatoWorld, _format: string) {
...
Then(
  'no full-screen flicker \\(clear-screen sequence) occurs between frames',
  function (this: ChromatoWorld) {
    ...
    const tuiSource = fs.readFileSync(path.resolve('src/adapters/tuiAdapter.tsx'), 'utf-8');
    ...
    const hasDirectStdoutWrite = /process\.stdout\.write\s*\((?![^)]*ALTERNATE_SCREEN)/.test(
      tuiSource,
    );
    assert.ok(
