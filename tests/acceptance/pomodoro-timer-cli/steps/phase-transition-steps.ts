...
Given(
  'the developer completed {int} work session and its break',
  async function (this: ChromatoWorld, count: number) {
    const stateDir = path.join(this.tempDir, 'chromato');
    fs.mkdirSync(stateDir, { recursive: true });
    fs.writeFileSync(
      path.join(stateDir, 'state.json'),
...
Given(
  'Marcus has completed {int} work sessions and their short breaks',
  async function (this: ChromatoWorld, count: number) {
    const stateDir = path.join(this.tempDir, 'chromato');
    fs.mkdirSync(stateDir, { recursive: true });
...
Given(
  'Kai started a {int}-minute work session {int} minutes ago',
  function (this: ChromatoWorld, totalMinutes: number, elapsedMinutes: number) {
...
Given(
  'Kai started a {int}-second work session {int} seconds ago',
  function (this: ChromatoWorld, totalSeconds: number, elapsedSeconds: number) {
...
When(
  'the developer views the TUI at any point during the session',
  async function (this: ChromatoWorld) {
...
Then(
  ...
  function (this: ChromatoWorld, fromPhase: string, toPhase: string) {
    ...
    assert.ok(
      this.capturedOutput.includes(toPhase) || this.process !== null, // If the process is running, the transition will occur
...
Then(
  'the phase color scheme changes from work colors to break colors',
...
Then('the display shows {string}', function (this: ChromatoWorld, text: string) {
  ...
  const hasInState =
    state && Object.values(state).some((v) => String(v).includes(text.replace(/\D/g, '')));
...
Then(
  'the state file records {int} completed session for today',
...
Then('the next session would start as {string}', function (this: ChromatoWorld, badge: string) {
  const state = readStateFile(this);
  if (state) {
...
Then(
  'the progress bar fill covers approximately {int} percent of the bar width',
...
Then('the timer displays {string} remaining', function (this: ChromatoWorld, timeStr: string) {
...
Then('the badge {string} is visible in the display', function (this: ChromatoWorld, badge: string) {
  assert.ok(
...
Then('the badge does not disappear when the progress bar updates', function (this: ChromatoWorld) {
...
Then(
  ...
  function (this: ChromatoWorld, minutes: number, label: string) {
    ...
    if (this.capturedOutput) {
      assert.ok(
        this.capturedOutput.includes(label) || this.capturedOutput.match(new RegExp(label, 'i')),
...
Then(
  'the color scheme is visually distinct from the short break colors',
  function (this: ChromatoWorld) {
...
Then(
  'the phase label reads {string} not {string}',
  function (this: ChromatoWorld, expected: string, unexpected: string) {
    if (this.capturedOutput) {
      assert.ok(
...
Then(
  'the {string} value has increased by {int}',
  function (this: ChromatoWorld, field: string, _increment: number) {
    const state = readStateFile(this);
    assert.ok(state !== null, 'State file not found');
    // Presence of the field confirms it was updated.
    const camel = field.replace(/_([a-z])/g, (_, c) => c.toUpperCase());
    assert.ok(field in state || camel in state, `Expected field "${field}" in state file`);
  },
);

Then(
  'the {string} value reflects the current consecutive day count',
  function (this: ChromatoWorld, field: string) {
    const state = readStateFile(this);
    assert.ok(state !== null, 'State file not found');
    const camel = field.replace(/_([a-z])/g, (_, c) => c.toUpperCase());
...
Then(
  'the fill percentage is within {int} percent of the actual elapsed fraction',
  function (this: ChromatoWorld, tolerancePercent: number) {
    const state = readStateFile(this);
    assert.ok(state !== null, 'State file not found');
