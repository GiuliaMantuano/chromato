...
Given(
  ...
  function (this: ChromatoWorld, value: string) {
...
Given(
  "Priya's terminal is set to TERM=xterm without Unicode support",
  function (this: ChromatoWorld) {
    this.chromatoEnv = {
      ...this.chromatoEnv,
      TERM: 'xterm',
...
Given(
  'the TERM environment variable is set to {string}',
  function (this: ChromatoWorld, term: string) {
...
Given('the terminal environment reports TERM=dumb and LC_ALL=C', function (this: ChromatoWorld) {
  this.chromatoEnv = {
    ...this.chromatoEnv,
    TERM: 'dumb',
    LC_ALL: 'C',
...
Given(/^the terminal supports 256 colors \(TERM=xterm-256color\)$/, function (this: ChromatoWorld) {
...
Given(
  'the terminal environment reports TERM=dumb with no color support',
  function (this: ChromatoWorld) {
    this.chromatoEnv = {
      ...this.chromatoEnv,
      TERM: 'dumb',
...
Given(
  'Aiko has a terminal window set to {int} columns',
  function (this: ChromatoWorld, columns: number) {
    this.chromatoEnv = { ...this.chromatoEnv, COLUMNS: String(columns) };
  },
);

Given('a terminal window with {int} columns', function (this: ChromatoWorld, columns: number) {
  this.chromatoEnv = { ...this.chromatoEnv, COLUMNS: String(columns) };
});
 ...
 */
function writeWorkSessionState(
  world: ChromatoWorld,
...
When(
  'the developer runs {string} for the first time',
...
Given(
  '{word} has chromato installed and tmux {float} running',
  function (this: ChromatoWorld, _persona: string, _tmuxVersion: number) {
...
Given(
  'he copies the README tmux integration example verbatim into his tmux configuration',
  function (this: ChromatoWorld) {
...
Then(
  'the chromato widget appears and functions identically to the tmux 3.x behavior',
  function (this: ChromatoWorld) {
...
Given(
  'a work session with {int} seconds remaining is active',
  function (this: ChromatoWorld, remainingSeconds: number) {
...
When(
  'tmux evaluates {string} from a non-interactive shell',
  async function (this: ChromatoWorld, command: string) {
