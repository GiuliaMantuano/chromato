...
When(
  'the developer runs {string} and observes the Full TUI simultaneously',
...
When(
  'the developer reads the remaining time from {string}',
  async function (this: ChromatoWorld, command: string) {
    const args = command
      .replace(/^chromato\s+/, '')
      .split(/\s+/)
...
Then(
  'the output is a non-empty string containing Pomodoro phase and remaining time',
  function (this: ChromatoWorld) {
    assert.ok(
      this.capturedOutput.trim().length > 0,
...
Then(
  'the visible text length of the output is 20 characters or fewer',
  function (this: ChromatoWorld) {
    const visible = stripAnsi(this.capturedOutput.trim());
    assert.ok(
      visible.length <= 20,
...
Then(
  'when {string} is passed the output fits within {int} characters',
  async function (this: ChromatoWorld, flagsStr: string, maxChars: number) {
...
Then(
  'the command completes in under {int} milliseconds',
...
Then(
  'the output completes in under {int} milliseconds',
  function (this: ChromatoWorld, maxMs: number) {
    const SPAWN_OVERHEAD_BUDGET_MS = 150;
    const effectiveLimit = maxMs + SPAWN_OVERHEAD_BUDGET_MS;
    assert.ok(
...
Then(
  'the output is an empty string or a configured idle indicator',
  function (this: ChromatoWorld) {
    // Either empty or a short idle indicator (not a full session string).
    const trimmed = stripAnsi(this.capturedOutput.trim());
    const isEmptyOrIdle = trimmed.length === 0 || /idle|--/i.test(trimmed);
    assert.ok(isEmptyOrIdle, `Expected empty string or idle indicator but got: "${trimmed}"`);
  },
);

Then(
  'the visible text still shows phase and remaining time information',
  function (this: ChromatoWorld) {
    const visible = stripAnsi(this.capturedOutput.trim());
    assert.match(
      visible,
...
Then('the output is an empty string', function (this: ChromatoWorld) {
  const trimmed = stripAnsi(this.capturedOutput.trim());
  assert.strictEqual(trimmed, '', `Expected empty string output but got: "${trimmed}"`);
});

Then(
  'both outputs use the same work phase color scheme \\(green or cyan\\)',
  function (this: ChromatoWorld) {
    ...
    const hasWorkColor = /colour(4[0-9]|8[0-7])|cyan|green|\[32m|\[36m/i.test(this.capturedOutput);
...
Then('both remaining time values differ by at most 1 second', function (this: ChromatoWorld) {
...
Then(
  'the next call to {string} returns break phase color',
  async function (this: ChromatoWorld, command: string) {
    const args = command
      .replace(/^chromato\s+/, '')
      .split(/\s+/)
...
Then('the break timer shows {string} remaining', function (this: ChromatoWorld, time: string) {
  assert.ok(
...
Then(
  'no silent failure or empty output occurs due to PATH differences',
  function (this: ChromatoWorld) {
    assert.ok(
      this.capturedOutput.trim().length > 0,
      'Expected non-empty output from status command in non-interactive shell but got empty',
    );
    assert.ok(this.exitCode === 0, `Expected exit code 0 but got ${this.exitCode}`);
  },
);
...
Then(
  'the chromato widget appears in his status bar within {int} seconds',
  function (this: ChromatoWorld, _seconds: number) {
...
Then('the widget shows an idle indicator if no session is active', function (this: ChromatoWorld) {
...
Then('the state file contains field {string}', function (this: ChromatoWorld, fieldName: string) {
