 ...
 *
 * Behavior: `chromato status --format tmux` must complete in <50ms from cold spawn.
 * If Ink/React were eagerly imported at the top of index.ts, the cold-start cost
 * of 15-20ms would push the status command over the 50ms threshold (AC-03.1).
 *
 * Test Budget: 1 distinct behavior x 2 = 2 max unit tests.
 * (1) status command completes in under 50ms (cold spawn, no Ink/React cost)
 ...
 * observable since it is the documented AC-03.1 performance contract.
 */
...
// AC-03.1: status command must complete in under 50ms on a standard runner.
...
const STATUS_COLD_START_LIMIT_MS = 50;
const SAMPLES = 3;

describe('chromato status driving port -- cold-start performance', () => {
  it('completes in under 50ms from cold spawn (no eager Ink/React loading)', () => {
    const times: number[] = [];
